...
# make a composite spark for content per cycle from the fastp json file
# spark_content <- function(json) {
# 	j <- jsonlite::fromJSON(json)
# 	at_vals <- j$read1_before_filtering$content_curves$A + j$read1_before_filtering$content_curves$T
# 	gc_vals <- j$read1_before_filtering$content_curves$C + j$read1_before_filtering$content_curves$G
# 	
# 	
# 	at_spk <- sparkline(at_vals, 
# 										 lineColor = "#d7191c", 
# 										 lineWidth = 2,
# 										 width = 220,
# 										 height = 40,
...
# 										 fillColor = FALSE)
# 	
# 	comp_spk <- spk_composite(at_spk, gc_spk)
# 	as.character(htmltools::as.tags(comp_spk))
# 	
# }

# make a q score per cycle spark from the fastp json file
# spark_q_percycle <- function(json) {
# 	j <- jsonlite::fromJSON(json)
# 	values <- j$read1_before_filtering$quality_curves$mean
# 	
# 	spk_chr(paste( seq(1, length(values)), ":", values, sep = "" ), 
# 					lineWidth = 2,
# 					fillColor = "#D6EAF8",
# 					spotColor = FALSE,
# 					minSpotColor = FALSE,
# 					maxSpotColor = FALSE,
# 					width = 220, height = 40,
# 					tooltipFormat = "<span style='color: {{color}}'>&#9679;</span> {{prefix}}cycle: {{x}} meanq: {{y}} {{suffix}}</span>"
# 					)
# }
...
gc_density <- 
	mclapply(1:length(fqfiles), function(y) { faster_gc(fqfiles[y]) })

q_score_density <- 
	mclapply(1:length(fqfiles), function(y) { faster_qscore(fqfiles[y]) })
...
gc_df <- data.frame(
	...
	#q_score_percycle = sapply(jsonfiles, spark_q_percycle)
